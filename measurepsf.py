...
def getEllipticity(data):
    ...
    for trials in range(100):
        ...
        for i in range(size[0]):
            for j in range(size[1]):
                #temp1 = -((i-medx)**2/alphax/alphax-2.0*alphaxy/alphax/alphax/alphay/alphay*(i-medx)*(j-medy)+(j-medy)**2/alphay/alphay)
                #temp2 = (2*math.pi*alphax*alphay*math.sqrt(1-(alphaxy/alphax/alphay)**2))
                #print temp2
                #temp3 = temp1/2.0/(1-(alphaxy/alphax/alphay)**2)/temp2
        ...
        alphay=math.sqrt(resulty*2.0)
        alphaxy=covxy*2.0
    return e1, e2, ellip, -pa, flux,rms, converge, alphax, alphay
